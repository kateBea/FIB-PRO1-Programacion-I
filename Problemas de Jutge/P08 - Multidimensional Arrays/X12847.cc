...
int calcular_distancia(const Board& tauler, int i0, int j0) {
	...
	while (dist <= 10) {
		if (i0 - dist >= 0) {
			if (trobar_horizontal(tauler, i0 - dist, max(0, j0 - dist), min(j0 + dist, N - 1))) 
				return dist;
		}
		...
		if (j0 - dist >= 0) {
			if (trobar_vertical(tauler, j0-dist, max(0, i0 - dist), min(i0 + dist, N-1))) 
				return dist;
		}
